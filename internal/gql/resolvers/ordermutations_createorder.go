...
func (r *orderMutationsResolver) CreateOrder(ctx context.Context, obj *model.OrderMutations, input model.CreateOrderInput) (model.CreateOrderResult, error) {
	...
	var orderMessages []*apiprotov1.OrderMessage
	for _, candle := range product {
		orderMessage := &apiprotov1.OrderMessage{
			Id:          candle.ID.String(),
			Title:       candle.Title,
			Slug:        candle.Slug,
			Version:     uint32(candle.Version),
			Images:      candle.Images,
			ColorId:     candle.ColorID.String(),
			CategoryId:  candle.CategoryID.String(),
			Price:       int32(candle.Price),
			Weight:      int32(candle.Weight),
			Description: candle.Description,
		}
		orderMessages = append(orderMessages, orderMessage)
	}
	resp, err := r.env.GrpcOrder.IsOrderCreated(ctx, orderMessages)
	if err != nil {
		r.env.Logger.Warn("Failed create order grpc", err.Error())
		return nil, err
	}
